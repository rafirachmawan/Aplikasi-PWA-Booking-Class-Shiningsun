...
export function NotificationBell({ role }: NotificationBellProps) {
  ...
  const [hasNew, setHasNew] = useState(true); // show red dot initially
  ...
  const handleOpen = async () => {
    ...
    if (opening) {
      setHasNew(false); // dismiss red dot when opened
      if (!hasFetched) {
        setIsLoading(true);
        try {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown Panel */}
      {isOpen && (
        <div className="absolute right-0 top-12 w-80 sm:w-96 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-200 z-50">
          {/* Header */}
          ...
          <div className="max-h-[380px] overflow-y-auto divide-y divide-slate-50 dark:divide-slate-800">
            {isLoading && activities.length === 0 ? (
              ...
              activities.map((act: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    {/* Avatar / Icon */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        {/* Status badge */}
                        <span className={`inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded ${statusColor} text-white`}>
          ...
          {activities.length > 0 && (
            <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
              <p className="text-[10px] text-slate-400 text-center">
                Menampilkan {activities.length} aktivitas terbaru • Data hanya diambil saat lonceng diklik
              </p>
            </div>
          )}
        </div>
