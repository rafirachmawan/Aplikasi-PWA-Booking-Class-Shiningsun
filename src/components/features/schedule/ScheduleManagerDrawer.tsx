...
export function ScheduleManagerDrawer({
  ...
  const executeCancelBooking = async () => {
    ...
    try {
      ...
      setErrorMessage(
        "Gagal mengeluarkan siswa: " + (error.message || "Terjadi kesalahan."),
      );
    } finally {
      setIsDeleting(false);
    }
          ...
          <div className="relative z-10 w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex flex-col items-center pt-8 pb-4 px-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 text-center leading-relaxed">
                ...
                <strong className="text-slate-700 dark:text-slate-200">
                  {lockConfirm.className}
                </strong>{" "}
                pukul{" "}
                <strong className="text-slate-700 dark:text-slate-200">
                  {lockConfirm.time} WIB
                </strong>{" "}
                akan dikunci.
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-2 text-center bg-amber-50 dark:bg-amber-900/30 px-3 py-2 rounded-lg border border-amber-200 dark:border-amber-800">
                ⚠️ Siswa baru tidak dapat melakukan booking pada sesi yang
                dikunci.
              </p>
            </div>
            <div className="flex gap-3 px-6 pb-6">
              ...
              <button
                type="button"
                onClick={() =>
                  executeLock(lockConfirm.slotId, lockConfirm.currentStatus)
                }
                disabled={isLocking}
            ...
            {existingSlots.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-sm text-slate-500">
                  Belum ada kelas terjadwal pada hari ini.
                </p>
              </div>
            ) : (
              existingSlots.map((slot) => {
                ...
                return (
                  <div
                    key={slot.id}
                    className={`p-4 rounded-xl border bg-white dark:bg-slate-900 shadow-sm transition-all ${slot.is_locked ? "border-slate-200 opacity-75" : "border-slate-200 dark:border-slate-800"}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4
                          className={`font-semibold ${slot.is_locked ? "text-slate-500" : "text-slate-900 dark:text-white"}`}
                        >
                          {slot.class.name}
                        </h4>
                            ...
                            <span className="text-slate-900 dark:text-slate-100 font-extrabold truncate max-w-50 flex items-center gap-2">
                              {b.student?.label && (
                                <span
                                  ...
                                  style={{
                                    backgroundColor: b.student.label.hex_color,
                                  }}
                                ></span>
                              )}
                              <span>
                                {b.student?.status === "CG" && (
                                  <strong className="text-amber-600 dark:text-amber-500 mr-1">
                                    (CG)
                                  </strong>
                                )}
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="text-sm font-medium text-slate-600 dark:text-slate-400">
                        Sisa Kuota:{" "}
                        <strong className="text-slate-900 dark:text-white">
                          {slot.class.max_quota - bookedCount}
                        </strong>{" "}
                        kursi
                      </div>
                    </div>
